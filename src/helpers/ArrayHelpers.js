...
export const getArrayChildren = (arrays, type, { HourMax, HourMin }) => {
  ...
  const newScheduleTime = newSchedule.filter((item) => {
  ...
  const WrapSchedule = [];
  for (let schedulteTime of newScheduleTime) {
    const isSome = WrapSchedule.some((o) =>
      o.some((x) => x.TeacherID === schedulteTime.TeacherID)
    );
    if (!isSome) {
      const newArray = newScheduleTime.filter(
        (x) => x.TeacherID === schedulteTime.TeacherID
      );
      WrapSchedule.push(newArray);
    }
  }

  return WrapSchedule;
};
export const getArraysChildren = (arrays, type, { HourMax, HourMin }) => {
  let newHourMin = HourMin;
  let newHourMax = HourMax;
  if (type === "S") {
    newHourMax = "11:59:00";
...
export const getMaxSession = (Dates, HourSchool) => {
  ...
  for (let date of Dates) {
    if (date?.IndexList?.length > 0 || date?.DayItems?.length > 0) {
      let MaxItem = 0;
      if (
        getArrayChildren(date?.IndexList || date?.DayItems, "S", HourSchool)
          .length >
        getArrayChildren(date?.IndexList || date?.DayItems, "C", HourSchool)
          .length
      ) {
        MaxItem = getArrayChildren(
          date?.IndexList || date?.DayItems,
          "S",
          HourSchool
        ).length;
      } else {
